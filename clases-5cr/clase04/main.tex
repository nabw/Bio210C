...
\begin{frame}\frametitle{Scipy: Interpolación de datos}
    \only<1>{
        ...
        \end{center}
        Y si quiero saber la densidad en $t=3$? Eso es \emph{interpolación}\footnote{Extrapolación sería bajo 0 o sobre 8}
    }
    \begin{columns}
  ...
  > OD = np.array([0.1, 0.2, 0.5, 1.2, 1.5])

  > f = interpolate.interp1d(t, OD, kind)

  > tnew = np.linspace(0,8,100)
...
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t,fragile]\frametitle{Scipy: Cuándo se llega a 100 bacterias?}
    \begin{small}
    $$ f(t) \coloneqq 10 e^{0.5 t} - 100 = 0 $$
    \vspace{-0.5cm}
    \begin{columns}
        \begin{column}{0.49\textwidth}
    \begin{minted}{python}  
  from scipy.optimize import root_scalar 
  def f(t): return 10*np.exp(0.5*t) - 100
  domt = np.linspace(0, 6, 100)
  domP = [f(t) for t in domt]
  sol = root_scalar(f, bracket=[0, 6])
  plt.scatter(sol.root, f(sol.root))
  plt.plot(domt, domP);plt.show()
    ...
    \end{minted}
        ...
        \end{column}
    \end{columns}
    \pause\idea{Y si la población decrece?}
    \end{small}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t,fragile]\frametitle{Scipy: Temperatura óptima (Maximizando)}
    \begin{small}
    $$ \min_ T f(T) \coloneqq (T - 37)^2 - 100    $$
    \vspace{-0.5cm}
    \begin{columns}
        \begin{column}{0.49\textwidth}
    \begin{minted}{python}  
  from scipy.optimize import minimize_scalar 
  def f(T): return (T - 37)**2 - 100
  domT = np.linspace(20, 50, 100)
  domV = [f(T) for T in domT]
  sol = minimize_scalar(f)
  plt.scatter(sol.x, f(sol.x))
  plt.plot(domT, domV);plt.show()
    \end{minted}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{flushright}
